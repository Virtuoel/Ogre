...
public class SetStateCommand
{
	...
	public static void register(CommandDispatcher<ServerCommandSource> commandDispatcher_1)
	{
		...
		.then(CommandManager.argument("fluid", StateArgumentType.create()).executes((commandContext_1) ->
		{
			return run(commandContext_1.getSource(), BlockPosArgumentType.getLoadedBlockPos(commandContext_1, "pos"), StateArgumentType.getArgument(commandContext_1, "fluid"), (Predicate<CachedBlockPosition>) null);
		}).then(CommandManager.literal("keep").executes((commandContext_1) ->
		{
			return run(commandContext_1.getSource(), BlockPosArgumentType.getLoadedBlockPos(commandContext_1, "pos"), StateArgumentType.getArgument(commandContext_1, "fluid"), (cachedBlockPosition_1) ->
			{
	...
	private static int run(ServerCommandSource serverCommandSource_1, BlockPos blockPos_1, StateArgument blockArgument_1, @Nullable Predicate<CachedBlockPosition> predicate_1) throws CommandSyntaxException
	{
		...
		else if(!blockArgument_1.setState(serverWorld_1, blockPos_1, 2))
		{
			throw FAILED_EXCEPTION.create();
		}
		else
